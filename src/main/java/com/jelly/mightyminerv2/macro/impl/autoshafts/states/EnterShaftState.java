...
public class EnterShaftState implements AutoShaftState {
    ...
    /* ──────────────────────────────────────────────────────────────
       GLOBAL EARLY-EXIT:
       If the “Glacite Mineshaft” GUI is already open, jump straight
       to CLICKING_SHAFT_INV from whatever state we are currently in.
    ────────────────────────────────────────────────────────────── */
        if (enteringShaftState != EnteringShaftState.CLICKING_SHAFT_INV
                && InventoryUtil.getInventoryName()
                .toLowerCase()
        ...
        switch (enteringShaftState) {
            ...
            case FINDING_SHAFT:
                ...
                if (closestMineshaft != null) {
                    ...
                    swapState(EnteringShaftState.PATHING_TO_SHAFT, random.nextInt(1000) + 1000); // random delay between 1-2 seconds
                } else {
            ...
            case PATHING_TO_SHAFT:
                ...
                List<BlockPos> targets   = BlockUtil.getWalkableBlocksAround(blockUnderShaft);
                BlockPos target;
            ...
            case WAITING_FOR_PATH:
                if (PlayerUtil.getNextTickPosition()
                        .squareDistanceTo(this.closestMineshaft.getPositionVector()) < 6
                        || Pathfinder.getInstance().succeeded()) {

                    Pathfinder.getInstance().stop();
                    KeyBindUtil.releaseAllExcept(mc.gameSettings.keyBindUseItem); // stops movement, etc.
                ...
                if (Pathfinder.getInstance().failed()) {
                    ...
                    pathRetryCount++;
                    if (pathRetryCount > 3) {
                        swapState(EnteringShaftState.WALK_FORWARD, 0);
                    }
                }
                break;
            ...
            case WALK_FORWARD:
                ...
                if (RotationHandler.getInstance().isEnabled()
                        && rotating
                        && RotationHandler.getInstance().isFollowingTarget()) {
            ...
            case TIMEOUT:
                ...
                if (PlayerUtil.getNextTickPosition()
                        .squareDistanceTo(this.closestMineshaft.getPositionVector()) < 6) {
                    KeyBindUtil.releaseAllExcept(mc.gameSettings.keyBindUseItem);
                    swapState(EnteringShaftState.ROTATING_TO_SHAFT, 0);
            ...
            case ROTATING_TO_SHAFT:
                if (!Pathfinder.getInstance().isRunning()) {
                    if (mc.thePlayer.getDistanceSqToEntity(closestMineshaft) < 1
                            && (Objects.equals(mc.objectMouseOver.entityHit, closestMineshaft)
                            || Objects.equals(mc.objectMouseOver.entityHit, fullClosestMineshaft))) {
                    ...
                    RotationHandler.getInstance()
                            .easeTo(new RotationConfiguration(
                                    new Target(closestMineshaft),
                                    MightyMinerConfig.getRandomAotvLookDelay(),
                                    null));
            ...
            case ENTERING_SHAFT:
                if (timer.isScheduled() && timer.passed()) {
                    ...
                    /* Hold right-click again (it was released in WAITING_FOR_PATH) */
                    KeyBindUtil.setKeyBindState(mc.gameSettings.keyBindUseItem, true);
                ...
                /* GUI detection is now handled globally, so nothing else here */
                break;
            ...
            case CLICKING_SHAFT_INV:
                if (timer.isScheduled() && timer.passed()) {
                    ...
                    if (System.currentTimeMillis() - macro.getLastShaftEntranceTime() < 31_000) {
                        break;                              // cooldown still active
                    }

                    if (!InventoryUtil.getInventoryName()
                            .toLowerCase()
                            .contains("glacite mineshaft")) {
