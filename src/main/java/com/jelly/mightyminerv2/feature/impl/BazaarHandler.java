...
import com.jelly.mightyminerv2.util.helper.SignUtils;
...
public class BazaarHandler extends AbstractFeature {
    ...
    private void handleBuyFromBz() {
        // System.out.println("Current buyState: " + buyState);
        switch (this.buyState) {
            ...
            case OPEN_BZ:
                ...
                break;
            case BZ_VERIFY:
                if (!InventoryUtil.isInventoryLoaded()) return;
                if (this.openedChestGuiNameStartsWith("Bazaar ➜ \"")) {
                    log("Opened Bz.");
                    timer.schedule(MightyMinerConfig.getRandomGuiWaitDelay());
            ...
            case CLICK_ON_PRODUCT:
                //log("In click on product state");
                if (!hasTimerEnded()) {
                    //log("Waiting for timer to end");
                    break;
                }

                int itemSlot = InventoryUtil.getSlotIdOfItemInContainer(this.itemToBuy, true);
                log("Item slot: " + itemSlot);
                ...
                InventoryUtil.clickContainerSlot(itemSlot, InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
                timer.schedule(2000);
            ...
            case CLICK_BUY_INSTANTLY:
                ...
                int buyInstantlySlot = InventoryUtil.getSlotIdOfItemInContainer("Buy Instantly");
                if (buyInstantlySlot == -1) {
                    this.disable("Cannot find buy instantly button. Disabling");
                    return;
                }
                InventoryUtil.clickContainerSlot(buyInstantlySlot, InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
            ...
            case BUY_INSTANTLY_VERIFY:
                ...
                if (this.openedChestGuiNameStartsWith(this.itemToBuy + " ➜ Instant Buy")) {
                    ...
                    List<Slot> buySlots = InventoryUtil.getIndexesOfItemsFromContainer(buyPredicate);

                    if (buySlots.isEmpty()) return;

                    for (Slot slot : buySlots) {
                        String lore = String.join(" ", InventoryUtil.getItemLore(slot.getStack()));
                    ...
                    Slot customAmount = InventoryUtil.getSlotOfItemInContainer("Custom Amount");
            ...
            case OPEN_SIGN:
                ...
                Slot signSlot = InventoryUtil.getSlotOfItemInContainer("Custom Amount");
                if (signSlot == null || !signSlot.getHasStack()) {
                    this.disable("Could not find sign.");
                    return;
                }

                InventoryUtil.clickContainerSlot(signSlot.slotNumber, InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
                timer.schedule(2000);
            ...
            case VERIFY_CONFIRM_PAGE:
                ...
                if (this.openedChestGuiNameContains("Confirm Instant Buy")) {
                    ...
                    this.buyNowButtonSlot = InventoryUtil.getSlotIdOfItemInContainer("Custom Amount");
                    timer.schedule(MightyMinerConfig.getRandomGuiWaitDelay());
                    this.buyState = BuyState.CLICK_BUY;
                }
                if (hasTimerEnded()) {
            ...
            case CLICK_BUY:
                ...
                Slot slot = InventoryUtil.getSlotOfIdInContainer(buyNowButtonSlot);
                ...
                String lore = String.join(" ", InventoryUtil.getItemLore(slot.getStack())).replace(",", "");
                ...
                if (matcher.find()) {
                    ...
                    if (this.maxSpendLimit > 0 && amount > this.maxSpendLimit) {
                        Logger.sendError("[AutoBazaar] Declining to buy due to exceeding the price limit. Item price: " + amount + ", limit: " + this.maxSpendLimit);
                ...
                InventoryUtil.clickContainerSlot(slot.slotNumber, InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
                timer.schedule(2000);
                break;
            case BUY_VERIFY:
                if (!InventoryUtil.isInventoryLoaded()) return;
            ...
            case WARNING_PAGE:
                if (!InventoryUtil.isInventoryLoaded()) return;
                if (this.openedChestGuiNameContains("Confirm") && !this.openedChestGuiNameContains("Instant Buy")) {
                    if (InventoryUtil.getSlotIdOfItemInContainer("WARNING") != -1) {
                        timer.schedule(1000);
                    } else if (InventoryUtil.getSlotIdOfItemInContainer("Confirm") != -1) {
                        this.buyState = BuyState.BUY_VERIFY;
                        InventoryUtil.clickContainerSlot(InventoryUtil.getSlotIdOfItemInContainer("Confirm"), InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
                        timer.schedule(2000);
                        log("Clicked confirm.");
                    }
                    break;
    ...
    private void handleSellToBz() {
        switch (this.sellState) {
            ...
            case CLICK_INSTASELL:
                ...
                int itemSlot = InventoryUtil.getSlotIdOfItemInContainer(this.instaSellBtn[sellType], true);
                if (itemSlot == -1) {
                ...
                String lore = String.join(" ", InventoryUtil.getLoreOfItemInContainer(itemSlot));
            ...
            case CLICK_CONFIRM_INSTASELL:
                ...
                int instaSellConfirmSlot = InventoryUtil.getSlotIdOfItemInContainer("Selling whole inventory");
                if (instaSellConfirmSlot == -1) {
                    this.disable("Cannot find instasell confirm btn.");
                    return;
                }

                InventoryUtil.clickContainerSlot(instaSellConfirmSlot, InventoryUtil.ClickType.LEFT, InventoryUtil.ClickMode.PICKUP);
                timer.schedule(2000);
    ...
    private boolean openedChestGuiNameContains(String guiName) {
        String openGuiName = InventoryUtil.getInventoryName();
